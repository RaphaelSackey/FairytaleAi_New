		...
		<div className='h-16 container mx-auto sticky top-3 mb-14 z-50'>
			<div className='floating_nav h-full mx-[10%] rounded-full items-center flex justify-between border bg-white dark:bg-background'>
				...
				<div className='nav_right_wrapper gap-3 flex mx-3'>
					<Link
						href='/signInOut'
						className='rounded-full border px-3 transition ease-in-out hover:border-2 flex justify-center items-center'>
						{loggedInStatus ? "Sign out" : "Sign in"}
					</Link>
					<Link
						href='/createStory'
						className='rounded-full border  px-3 bg-gradient-to-r from-callToAction transition ease-in-out hover:bg-callToAction duration-500 flex justify-center items-center'>
						Create Story
					</Link>
					<DarkModeToggle />
				</div>
